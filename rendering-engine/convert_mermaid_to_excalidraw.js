// Polyfill self for svg-to-excalidraw
global.self = global;
...
// Polyfill DOMMatrix if available (JSDOM usually doesn't have it, but good to check)
if (window.DOMMatrix) {
  global.DOMMatrix = window.DOMMatrix;
}

const run = async () => {
  const mod = await import("svg-to-excalidraw");
  // Handle potentially nested default export
  let svgToEx = mod.default;
  if (svgToEx && svgToEx.default) {
    svgToEx = svgToEx.default;
  }

  const heartSVG = `<?xml version="1.0" encoding="UTF-8"?>
...
`;
  ...
  if (hasErrors) {
    ...
    console.log("Excalidraw JSON content:");
    console.log(content);
    // navigator.clipboard.writeText(content); // Not available in Node.js
  }
};
